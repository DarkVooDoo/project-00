...
var Test []model.Appointment = []model.Appointment{model.Appointment{Id: "3", CustomerName: "John Doe", Service: "Barbe", Date: "9:30 - 10:00"}, 
model.Appointment{Id: "83", CustomerName: "Alice Droux", Service: "Coupe", Date: "4:10 - 4:40"}}
...
func (p PlanningPayload) Get(w http.ResponseWriter, r *http.Request){
    ...
    planning := model.Appointment{Date: time.Now().Format(time.DateOnly), EmployeeId: "1"}
    p.Schedule = planning.EmployeePlanning()
...
func (p PlanningPayload) Post(w http.ResponseWriter, r *http.Request){
    ...
    var planning model.Appointment
            ...
            <div class="card {{if eq .Status "Happend"}}happend{{else if eq .Status "Now"}}now{{end}}" id="appointment{{.Id}}">
              ...
              <div class="footer">
                {{if eq .Status "Happend"}}
                    <button type="button" class="footerBtn">Reserve a nouveau</button>
                {{else if eq .Status "Next"}}
                    <button type="button" class="footerBtn btn-danger" onclick="onCancelAppointment({{.Id}})">Annuler</button>
                    <button type="button" class="footerBtn btn-primary">Modifier</button>
                {{end}}
              </div>
            </div>
        {{else}}
