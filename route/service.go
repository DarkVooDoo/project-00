...
func (s *ServiceRoute) Get(w http.ResponseWriter, r *http.Request){
    ...
    defer conn.Close()
    cookie, err := r.Cookie("eid")
    if err != nil{
        log.Printf("no etablishment cookie")
        return
    }
    etablishmentList := model.Service{EtablishmentId: cookie.Value}
    list, _ := etablishmentList.GetList(conn)
    serviceTemplate, _ := template.ParseGlob("view/Service.tmpl")
...
func (s *ServiceRoute) Post(w http.ResponseWriter, r *http.Request){
    ...
    service.EtablishmentId = cookie.Value
    ...
    temp, err := template.New("service").Parse(`
        <form class="service" hx-put="/etablissement/service" hx-swap="none" hx-ext="json-enc-custom" hx-vals='{"id": "{{.Id}}"}'>
                <input type="text" value="{{.Name}}" id="name" name="name" class="input" autocomplete="off" placeholder="Nom du service" />
                <textarea class="description" maxlength="150">{{.Description}}</textarea>
                <div class="more">
                    <input type="number" value="{{.Duration}}" id="duration" name="duration" class="input" autocomplete="off" step="1" placeholder="Duration (Min)" />
                    <input type="number" value="{{.Price}}"  id="price" name="price" class="input" autocomplete="off" step=".01" placeholder="Prix" />
                </div>
            <div class=command>
...
func (s *ServiceRoute) Delete(w http.ResponseWriter, r *http.Request){
    ...
    service := model.Service{Id: string(by)[3:]}
