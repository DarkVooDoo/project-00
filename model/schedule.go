...
func (s DaySchedule) Create(conn *sql.Conn)error{
	...
	scheduleRow := conn.QueryRowContext(context.Background(), `INSERT INTO schedule (day, open_time, etablishment_id) VALUES($1,$2,$3) RETURNING id`, 
	s.Day, s.OpenTime, s.EtablishmentId)
...
func (s DaySchedule) GetSchedule(conn *sql.Conn)[]DaySchedule{
	...
	scheduleRows, err := conn.QueryContext(context.Background(), `SELECT id, day, TO_CHAR(open_time, 'HH24:MI'), COALESCE(TO_CHAR(close_time, 'HH24:MI'), ''),
	 CASE WHEN open_time > close_time AND EXTRACT(ISODOW FROM NOW()) - 1 = day THEN
		...
		ELSE '' END, ('{"Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche"}'::TEXT[])[day+1] FROM schedule WHERE etablishment_id=$1 ORDER by day`, s.EtablishmentId)
	if err != nil{
		log.Printf("error getting the schedule: %s", err)
		return schedule
	}
	for scheduleRows.Next(){
		if err := scheduleRows.Scan(&s.Id, &s.Day, &s.OpenTime, &s.CloseTime, &s.IsClose, &s.DayName); err != nil{
...
func(s DaySchedule) Delete()error{
	...
	result, err := conn.ExecContext(context.Background(), `UPDATE schedule SET close_time=NULL WHERE id=$1 AND etablishment_id=$2`, s.Id, s.EtablishmentId)
	if err != nil{
